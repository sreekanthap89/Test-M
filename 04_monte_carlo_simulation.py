...
POOL       = 40
DRAW_SIZE  = 6
N_SIMULATIONS = 50_000    # number of simulated draws
SEED       = 42
...
def build_probability_vector(df) -> np.ndarray:
    """
    Build an empirical probability vector for numbers 1-40
    ...
    for row in df["numbers"]:
        for n in row:
            freq[n - 1] += 1
    # Normalise to a proper probability vector (sums to 1)
...
def simulate_draws(prob_vector: np.ndarray, n_sims: int, strategy: str) -> list:
    ...
    if strategy == "uniform":
        weights = None     # np.random treats None as uniform
    else:
        weights = prob_vector
...
def main():
  ...
  Example: "If I buy a ticket with numbers {{1,7,13,20,33,39}},
            what is the probability of matching 3 out of 6?"
...
""")
    ...
    # Use the LAST ACTUAL draw as our "target ticket"
...
""")
  ...
  Theoretical expected matches = 6 × (6/40) = {:.4f}
""".format(DRAW_SIZE * DRAW_SIZE / POOL))

    # Running mean of uniform match scores
  ...
  This is more flexible than formula-based CIs for complex situations.
""")
...
""")
    ...
    fig.suptitle("STEP 4 — Monte Carlo Simulation", fontsize=15, fontweight="bold")
