"""
=============================================================
 STEP 5: MARKOV CHAIN TRANSITION MODELLING
=============================================================
 LEARNING GOAL:
 ...
 KEY CONCEPTS INTRODUCED:
   ...
   * Application to number zones (1-10, 11-20, 21-30, 31-40)
=============================================================
"""
...
def zone_of(n: int) -> int:
    """Return zone index 0-3 for number n (1-40)."""
    return (n - 1) // 10          # 0=1-10, 1=11-20, 2=21-30, 3=31-40
...
def build_zone_transition_matrix(df) -> np.ndarray:
    """
    States = (count_z0, count_z1, count_z2, count_z3) where sum = 6.
    Builds a transition matrix T[i,j] = P(go to state j | currently in state i).
    Because the number of possible states is large, we discretise by
    'dominant zone' (the zone with the most numbers in a draw).
    """
    # Simpler state: dominant zone (0-3)
...
def build_sum_band_transition(df) -> np.ndarray:
    ...
    States = draw sum discretised into bands:
      Band 0: sum < 100  (low)
      Band 1: 100 ≤ sum < 130  (medium-low)
      Band 2: 130 ≤ sum < 160  (medium-high)
      Band 3: sum ≥ 160  (high)
    """
    bands = [0, 100, 130, 160, 999]
    labels = [0, 1, 2, 3]

    df = df.copy()
    df["sum"] = df["numbers"].apply(sum)
    df["band"] = pd.cut(df["sum"], bins=bands, labels=labels, right=False).astype(int)
...
def main():
  ...
  It massively simplifies prediction: you only need to know
  WHERE YOU ARE NOW, not the full path that got you here.

  For EASY6 we define states as:
    • 'Dominant Zone' — which zone (1-10, 11-20, 21-30, 31-40)
...
""")
  ...
  time the chain spends in each state.  If we ran the lottery
  forever, π tells us how often each zone would dominate.

  It is computed by solving:  π × T = π
  (π is the left eigenvector of T corresponding to eigenvalue 1)
""")
  ...
  As n → ∞, every row of T^n converges to the stationary π.
  This confirms that for large n, the starting state doesn't matter.
""")
    ...
    band_names = ["Low (<100)", "Med-Low (100-129)", "Med-High (130-159)", "High (≥160)"]
    ...
    print("""
  Does seeing a number in draw t make it MORE or LESS likely
  to appear in draw t+1?

  We compute, for each number n:
    P(n appears in draw t+1 | n appeared in draw t)
  vs
    P(n appears in draw t+1) — the baseline rate = 6/40
""")
    baseline  = 6 / 40
    ...
    print(f"  Baseline repeat probability : {baseline:.4f}  (6/40)")
    ...
    fig.suptitle("STEP 5 — Markov Chain Transition Modelling", fontsize=15, fontweight="bold")
    ...
    ax4.axhline(baseline * 100, color="black", linestyle="--", linewidth=1.5,
                label=f"Baseline (6/40 = {baseline*100:.1f}%)")
  ...
  ✔  How to build a transition matrix from real data
