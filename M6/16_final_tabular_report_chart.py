...
=============================================================
 LEARNING GOAL:
   ...
   (step14_final_tabular_report.png) summarizing all 16 steps,
   their real-world explanations, predicted tickets, win-rate metrics,
   EVT volatility bounds, GNN relational network, and final AI recommendations.
=============================================================
"""
...
def main():
    ...
    t15 = sorted((np.argsort(signals_dict["15. Gap Regularity"])[::-1][:DRAW_SIZE] + 1).tolist())
    t16 = sorted((np.argsort(signals_dict["18. GNN Relational Net"])[::-1][:DRAW_SIZE] + 1).tolist())
    
    top14_indices = np.argsort(meta_prob)[::-1][:14]
    pool14 = sorted((top14_indices + 1).tolist())
    t12 = mod14.select_optimal_ticket(pool14, meta_prob, ticket_size=DRAW_SIZE, df=df)
    top1_num = int(np.argmax(meta_prob) + 1)

    # Calculate validation depth & EVT volatility metrics
    chi2_res = chi_squared_fit_test(t12, df)
    pair_triple_res = calculate_pair_triple_match_rate(t12, df["numbers"].iloc[-1])
    ...
    table_data = [
        ...
        ["Step 07", "4-Phase Markov Engine", "Advanced 39x39 matrix with feedback loop", str(t7), "Streak & Zone Feedback Control"],
        ...
        ["Step 12", "Feature & Metric Depth", "Gap analysis, consecutive streaks, Chi2 fit", str(t15), f"Chi2 Fit Score: {chi2_res['statistical_fit_score']}/100"],
        ["Step 13", "GNN & Hawkes Meta", "Graph Neural Network + Hawkes-Jump + Neural Meta", str(t16), f"GNN Relational Net & Meta-Learner v3"],
        ["Step 14", "Master AI Meta V3", "Fuses 19 Signals via Neural Meta-Learner", str(t12), "GRAND UNIFIED META-AI TICKET 🏆"],
    ...
    fig.suptitle("EMIRATES DRAW EASY6 — COMPLETE A.I. PREDICTION & RANDOMNESS AUDIT REPORT (V3 FINAL)",
    ...
    summary_text_left = (
        ...
        f"   • EVT Ideal Sum Range       : {evt_vol['ideal_sum_range']} (P(Sum > 120) = {evt_vol['prob_high_sum_gt_120']}%)"
    )
    ...
    summary_text_right = (
        ...
        "   • Total Wheeled Tickets: 26 Tickets"
    )
    ...
    tips_text = (
        "BLACKROCK-INSPIRED INSTITUTIONAL QUANT TIPS & STRATEGY (EASY6 V3)\n\n"
        ...
        "3. GNN RELATIONAL MANIFOLDS: Utilize Graph Neural Network neighborhood embeddings over 39-node ball graph.\n"
        "4. HAWKES & JUMP-DIFFUSION RECOVERY: Model dormant cold numbers crossing threshold gap with Poisson hazard.\n"
        "5. EVT VOLATILITY CONSTRAINTS: Filter ticket sum totals within [87, 155] Pareto tail volatility bounds.\n"
    ...
    print("\n============================================================")
    print("  GRAND FINAL SUMMARY REPORT (EASY6 V3)")
    print("============================================================")
    print(f"★ ULTIMATE RECOMMENDED GRAND MASTER TICKET V3 : {t12} ★")
