"""
=============================================================
 STEP 2: FREQUENCY ANALYSIS — HOT & COLD NUMBERS (MEGA7)
=============================================================
 LEARNING GOAL:
...
POOL      = 37          # numbers 1-37
DRAWS_PER = 7           # 7 numbers per draw
...
def count_frequencies(df, weight_by_recency=False):
    """
    Count how many times each number 1-37 was drawn.
    If weight_by_recency=True, more recent draws get higher weight.
    """
    freq = np.zeros(POOL + 1, dtype=float)   # index 0 unused; 1..37
    ...
    for i, row in enumerate(df["numbers"]):
        ...
        for num in row:
            freq[num] += weight
    return freq[1:]   # return array index 0..36 → numbers 1..37
...
def main():
    ...
    print(f"""
  In {n_draws} draws of {DRAWS_PER} numbers each, total balls drawn = {n_draws*DRAWS_PER}.
...
""")
    ...
    expected = n_total_balls_drawn / POOL
  ...
  If p-value > 0.05  → we CANNOT reject H₀ (looks UNIFORM, consistent with randomness)
  If p-value < 0.05  → we REJECT H₀ (frequency distribution is NOT uniform — bias detected)
""")
    chi2_stat, p_value = chisquare(raw_freq, f_exp=np.full(POOL, expected))
    print(f"  χ² statistic : {chi2_stat:.3f}")
    print(f"  p-value      : {p_value:.4f}")
    if p_value > 0.05:
        print("  Verdict      : ✔ Cannot reject H₀ — draw appears UNIFORM (consistent with randomness)")
    else:
...
""")
    ...
    fig.suptitle("STEP 2 — Frequency Analysis: Hot & Cold Numbers (MEGA7)", fontsize=15, fontweight="bold")
    ...
    ax.set_title(f"Raw Frequency of Each Number (1–{POOL})")
    ...
    print("""
  ✔  How to compute empirical frequencies from real MEGA7 draw data
