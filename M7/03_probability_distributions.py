"""
=============================================================
 STEP 3: PROBABILITY DISTRIBUTIONS (MEGA7)
=============================================================
 LEARNING GOAL:
...
def main():
  ...
  For example:
    If number 7 appeared {int(freq[6])} times in {n_draws * DRAWS_PER} total draws:
      P(7) = {freq[6]}/{n_draws * DRAWS_PER} = {freq[6]/(n_draws*DRAWS_PER):.4f}
...
""")
    ...
    print(f"  {'Number':>6}  {'P(obs)':>8}  {'CI lower':>9}  {'CI upper':>9}  {'Includes 1/37?':>14}")
    for i in range(POOL):
    ...
    prob_pair      = pairs_per_draw / total_pairs
    print(f"""
  Some pairs of numbers may appear together more often than chance.
  If numbers A and B are truly independent in MEGA7:
    P(A and B in same draw) = C(35,5)/C(37,7) = 21/666 ≈ {prob_pair:.4f}
...
""")
    ...
    expected_co = n_draws * pairs_per_draw / total_pairs
    ...
    fig.suptitle("STEP 3 — Probability Distributions (MEGA7)", fontsize=15, fontweight="bold")
    ...
    ax2.plot([1, POOL], [0, 1], color="red", linestyle="--", linewidth=1.2, label="Uniform CDF")
    ...
    print("""
  ✔  Empirical probability from observed MEGA7 data
