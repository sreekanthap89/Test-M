"""
=============================================================
 STEP 6: COMBINED PREDICTION REPORT (MEGA7)
=============================================================
 LEARNING GOAL:
   Combine ALL signals from steps 1-5 into a single, structured
   prediction report for the NEXT Emirates Draw MEGA7.
...
def signal_frequency(df, recent_n=12) -> np.ndarray:
    """
...
def signal_pair_lift(df) -> np.ndarray:
    ...
    Numbers that frequently co-occur with last draw's numbers get higher scores.
    """
    ...
    scores    = np.zeros(POOL)
    expected  = len(df) * comb(DRAW_SIZE, 2) / comb(POOL, 2)
...
def main():
    ...
    print_section("COMBINED PREDICTION REPORT (MEGA7)")
    print(f"""
...
""")
    ...
    top_indices = sorted_by_combined[:14]
  ...
  Top 14 candidates (by combined score):
  {top_numbers}

  Suggested ticket (top 7 by combined score):
""")
    top7 = sorted([int(i + 1) for i in sorted_by_combined[:DRAW_SIZE]])
    print(f"  ★  {top7}  ★")
    print()
    ...
    mc_top7 = sorted(np.argsort(mc_freq)[::-1][:DRAW_SIZE] + 1)
    print(f"  MC-based top 7 (most simulated appearance): {mc_top7}")

    overlap = set(top7) & set(mc_top7)
    print(f"  Overlap between ensemble and MC top-7    : {sorted(overlap)}")

    # ── Confidence: how often does top-7 match the last real draw? ───────────
    print_section("BACK-TEST: Signal Quality on Historical Data")
    print("""
  Back-test: how many of the 'top 7 by combined score' do we
...
""")
    ...
    for test_i in range(test_start, n_draws):
        ...
        pred_top7 = sorted(np.argsort(comb_s)[::-1][:DRAW_SIZE] + 1)
        matches   = len(set(pred_top7) & set(actual))
        results.append(matches)

    avg_match = np.mean(results)
    print(f"  Back-test draws    : {len(results)}")
    print(f"  Avg matches (top-7 vs actual) : {avg_match:.2f} / 7")
    print(f"  Distribution of matches       : ", end="")
    for k in range(DRAW_SIZE + 1):
        pct = results.count(k) / len(results) * 100
        print(f"{k}={pct:.1f}%", end="  ")
    print()
    theoretical_match = DRAW_SIZE * DRAW_SIZE / POOL
    print(f"\n  Random baseline (uniform) would give avg ≈ {theoretical_match:.2f} matches")
    if avg_match > theoretical_match:
    ...
    fig.suptitle("STEP 6 — Combined Prediction Report\nEmirates Draw MEGA7", fontsize=15, fontweight="bold")
    ...
    for n in sorted_by_combined[:DRAW_SIZE]:
        ax1.axvline(n - 0.5, color="cyan", linewidth=0.8, alpha=0.5)
        ax1.axvline(n + 0.5, color="cyan", linewidth=0.8, alpha=0.5)

    # 6b — Combined probability bar chart
    ax2 = fig.add_subplot(gs[1, :])
    bar_colors = ["#e74c3c" if i in sorted_by_combined[:DRAW_SIZE] else "#4C72B0"
                  for i in range(POOL)]
    ...
    for idx in sorted_by_combined[:DRAW_SIZE]:
        ax2.text(idx + 1, combined[idx] * 100 + 0.05, f"#{idx+1}",
                 ha="center", fontsize=7, color="#e74c3c", fontweight="bold")
    ax2.set_title(f"Ensemble Probability per Number  |  ★ Red = Suggested top-7: {top7}")
    ...
    bar_c2     = ["#e74c3c" if n in top7 else "#4C72B0" for n in top20_nums]
    ...
    match_counts = [results.count(k) for k in range(DRAW_SIZE + 1)]
    ax4.bar(range(DRAW_SIZE + 1), match_counts, color="#55A868")
    ax4.axvline(avg_match, color="red", linestyle="--", linewidth=1.5,
                label=f"Mean = {avg_match:.2f}")
    ax4.axvline(theoretical_match, color="orange", linestyle=":", linewidth=1.5,
                label=f"Random baseline ({theoretical_match:.2f})")
    ax4.set_xticks(range(DRAW_SIZE + 1)); ax4.set_xticklabels([f"{k} matches" for k in range(DRAW_SIZE + 1)], fontsize=8)
    ax4.set_title(f"Back-test: Top-7 vs Actual\n(last {len(results)} draws)")
  ...
  Next draw suggested ticket: ★  {top7}  ★
