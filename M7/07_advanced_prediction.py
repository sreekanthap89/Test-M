"""
=============================================================
 STEP 7: ADVANCED PREDICTION — NUMBER-LEVEL MARKOV + FEEDBACK LOOP (MEGA7)
=============================================================
 LEARNING GOAL:
   ...
   GAP 1 — Number-level Markov Chain
     ...
     Here we build a FULL 37×37 transition matrix where
     each state is an individual number 1-37, and we track
...
# ─────────────────────────────────────────────────────────────────────────────
# PHASE 3: NUMBER-LEVEL MARKOV CHAIN
# ─────────────────────────────────────────────────────────────────────────────

def phase3_number_markov(df) -> np.ndarray:
    """
    Build a FULL 37x37 Markov transition matrix where:
...
def phase4_feedback_loop(df, base_prob: np.ndarray, lookback: int = 5) -> np.ndarray:
    """
    Detect recent streak patterns and adjust probabilities.
    """
    ...
    sum_pattern = "NONE"
    if all_high:
        # Boost low zones (Z1, Z2), dampen high zones (Z3, Z4)
        correction[:10] *= 1.15       # Z1: 1-10
        correction[10:20] *= 1.10     # Z2: 11-20
        correction[20:30] *= 0.90     # Z3: 21-30
        correction[30:] *= 0.85       # Z4: 31-37
        sum_pattern = f"HIGH SUM STREAK ({lookback} draws) -> boosting low zones (Z1+Z2)"
    elif all_low:
        # Boost high zones (Z3, Z4), dampen low zones (Z1, Z2)
        correction[:10] *= 0.85       # Z1: 1-10
        correction[10:20] *= 0.90     # Z2: 11-20
        correction[20:30] *= 1.10     # Z3: 21-30
        correction[30:] *= 1.15       # Z4: 31-37
        sum_pattern = f"LOW SUM STREAK ({lookback} draws) -> boosting high zones (Z3+Z4)"
...
# ─────────────────────────────────────────────────────────────────────────────
# COLD/DUE NUMBER SIGNAL
# ─────────────────────────────────────────────────────────────────────────────

def signal_cold_due(df, lookback: int = 10) -> np.ndarray:
    recent = set()
...
# ─────────────────────────────────────────────────────────────────────────────
# DIVERSITY-AWARE TICKET SELECTION
# ─────────────────────────────────────────────────────────────────────────────

def diversity_select(scores: np.ndarray, k: int = 7,
                     candidate_pool: int = 16) -> list[int]:
    """
    ...
    candidates = [int(idx + 1) for idx in ranked]
    candidate_scores = {int(idx + 1): scores[idx] for idx in ranked}
    ...
    for combo in combinations(candidates, k):
        ...
        gaps = [ticket[i+1] - ticket[i] for i in range(k-1)]
        n_consecutive = sum(1 for g in gaps if g <= 1)
        if n_consecutive > 1:
            continue
    ...
    for idx in ranked:
        n = int(idx + 1)
        if any(abs(n - p) <= 1 for p in picked):
            continue
        picked.append(n)
        if len(picked) == k:
            break
...
def validate_sum_range(ticket: list[int], scores: np.ndarray,
                       df_sums: pd.Series,
                       candidate_pool: int = 16) -> list[int]:
    q1 = df_sums.quantile(0.25)
    q3 = df_sums.quantile(0.75)
    ...
    ranked = np.argsort(scores)[::-1][:candidate_pool]
    available = [int(idx + 1) for idx in ranked if (idx + 1) not in ticket]
    target_sum = (q1 + q3) / 2

    best_ticket = ticket[:]
    best_dist   = abs(ticket_sum - target_sum)

    for i, old_n in enumerate(ticket):
        for new_n in available:
            trial = ticket[:]
            trial[i] = new_n
            trial_sorted = sorted(trial)

            gaps = [trial_sorted[j+1] - trial_sorted[j]
                    for j in range(len(trial_sorted)-1)]
            if sum(1 for g in gaps if g <= 1) > 1:
                continue
            zones = len(set((n-1)//10 for n in trial_sorted))
            if zones < 3:
                continue

            trial_sum = sum(trial_sorted)
            dist = abs(trial_sum - target_sum)
            if dist < best_dist:
                best_dist   = dist
                best_ticket = trial_sorted
...
def mc_confidence_interval(mc_freq: np.ndarray, n_sims: int,
                            top_k: int = 7, ci_level: float = 0.80) -> tuple:
...
def main():
    ...
    print_section("STEP 7 — ADVANCED PREDICTION (Full 4-Phase Framework - MEGA7)")
    print(f"""
  Dataset  : Emirates Draw MEGA7
  Draws    : {n_draws}
  Last draw: {last_date}  ->  {last_draw}

  This script implements the COMPLETE 4-phase framework for MEGA7:
    Phase 1: Weighted P_i from historical frequency
    Phase 3: Number-level 37x37 Markov transition matrix
    Phase 4: Feedback loop (streak detection + adjustment)
    Phase 2: Monte Carlo seeded with ensemble of Phases 1+3+4
""")
    ...
    # ── PHASE 3 ───────────────────────────────────────────────────────────────
    print_section("PHASE 3 — Number-Level 37x37 Markov Transition")
    ...
    # ── COLD/DUE SIGNAL ────────────────────────────────────────────────────────
    print_section("COLD/DUE SIGNAL — Numbers Missing from Recent Draws")
    p_cold = signal_cold_due(df, lookback=8)
    cold_top10 = np.argsort(p_cold)[::-1][:10]
    cold_numbers = [int(idx + 1) for idx in cold_top10 if p_cold[idx] > 0]
    print(f"  Numbers NOT seen in last 8 draws (candidates):")
    print(f"  {cold_numbers[:14]}")

    # ── ENSEMBLE: P1 + P3 + P4 + Cold ────────────────────────────────────────
    print_section("ENSEMBLE — Combining Phase 1 + 3 + 4 + Cold")
    W1, W3, W4, WC = DEFAULT_WEIGHTS["w1"], DEFAULT_WEIGHTS["w3"], DEFAULT_WEIGHTS["w4"], DEFAULT_WEIGHTS["wc"]
    ensemble_prob = (W1 * p1 + W3 * p3 + W4 * p4_adjusted + WC * p_cold)
    ensemble_prob = ensemble_prob / ensemble_prob.sum()

    print(f"  {'Num':>4}  {'P1 (Freq)':>10}  {'P3 (Markov)':>12}  {'P4 (Adj.)':>10}  {'Cold':>8}  {'Ensemble':>10}")
    print("  " + "-" * 70)
    ...
    for rank in range(15):
        ...
        print(f"  {n:4d}  {p1[idx]:10.5f}  {p3[idx]:12.5f}  {p4_adjusted[idx]:10.5f}  "
              f"{p_cold[idx]:8.4f}  {ensemble_prob[idx]:10.5f}{flag}")
    ...
    mc_prob  = mc_freq / mc_freq.sum()
    top7_mc_raw = sorted((np.argsort(mc_freq)[::-1][:DRAW_SIZE] + 1).tolist())

    print(f"  Top 7 by MC frequency (raw, before diversity filter): {top7_mc_raw}")

    # ── DIVERSITY SELECTION ────────────────────────────────────────────────────
    print_section("DIVERSITY FILTER — Zone Coverage + Anti-Clustering")
    top7_mc = diversity_select(ensemble_prob, k=DRAW_SIZE, candidate_pool=DEFAULT_POOL_SIZE)
    print(f"  After diversity filter: {top7_mc}")

    ticket_gaps = [top7_mc[i+1]-top7_mc[i] for i in range(DRAW_SIZE-1)]
    ticket_zones = len(set((n-1)//10 for n in top7_mc))
    print(f"  Gaps: {ticket_gaps}  |  Zones covered: {ticket_zones}/4")

    # ── SUM RANGE VALIDATION ──────────────────────────────────────────────────
    q1, q3 = df["sum"].quantile(0.25), df["sum"].quantile(0.75)
    ticket_sum_before = sum(top7_mc)
    top7_mc = validate_sum_range(top7_mc, ensemble_prob, df["sum"],
                                  candidate_pool=DEFAULT_POOL_SIZE)
    ticket_sum_after = sum(top7_mc)
    print(f"\n  Sum validation:")
    print(f"    Historical IQR: [{q1:.0f}, {q3:.0f}]")
    print(f"    Ticket sum before: {ticket_sum_before}  →  after: {ticket_sum_after}")
    if ticket_sum_before != ticket_sum_after:
        print(f"    Adjusted ticket: {top7_mc}")
    else:
    ...
    inclusion_prob, ci_numbers = mc_confidence_interval(mc_freq, N_MC,
                                                          top_k=DRAW_SIZE, ci_level=0.80)
    print(f"\n  80% Confidence Set (stable across 1000 bootstrap samples):")
    print(f"  {sorted(ci_numbers.tolist())}")
    print(f"\n  Inclusion probabilities for top-14 candidates:")
    top14_ci = np.argsort(inclusion_prob)[::-1][:14]
    for idx in top14_ci:
  ...
  PHASE-BY-PHASE SUMMARY:
   ...
   Phase 1  Weighted P_i top-7  : {sorted((np.argsort(p1)[::-1][:DRAW_SIZE]+1).tolist())}
   Phase 3  Markov top-7        : {sorted((top10_markov[:DRAW_SIZE]+1).tolist())}
   Phase 4  Feedback patterns   :
              Sum    : {sum_pattern}
              Zone   : {zone_pattern}
              Balance: {balance_pattern}
   Phase 2  Monte Carlo top-7   : {top7_mc}
...
""")
    ...
    ax1.set_title("Signal Heatmap: brighter = higher probability  |  cyan lines = top-7 prediction")
    ...
    bar_colors = ["#e74c3c" if (i+1) in top7_mc else "#4C72B0" for i in range(POOL)]
    ...
    ax3.set_title(f"37x37 Markov Transition Matrix\n(row=current, col=next)")
    ...
    bar_c = ["#e74c3c" if n in top7_mc else "#4C72B0" for n in top20_ci_nums]
    ...
    print("""
  GAP 1 FILLED:
  ✔  37x37 Markov transition matrix (number-level, not just zone)
  ...
  ✔  Phase 1: Weighted P_i
  ✔  Phase 2: Monte Carlo seeded with ensemble P + bootstrap CI
  ✔  Phase 3: Number-level Markov (37x37 T matrix from last draw)
