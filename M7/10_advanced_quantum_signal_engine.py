"""
=============================================================
 STEP 10: ADVANCED QUANTUM & SIGNAL SCIENCE ENGINE (MEGA7)
=============================================================
 LEARNING GOAL:
...
def signal_fft_spectral(df, pool_size=37):
    """
...
def signal_hawkes_process(df, pool_size=37, alpha=0.4, beta=0.2):
    """
    ...
    for b in range(1, pool_size + 1):
        ...
        # Sum memory kernel over previous draw arrival times t_k
...
def signal_maxent_entropy(df, pool_size=37, target_mean_sum=133.0):
    """
    Finds the unique probability distribution P* that maximizes Shannon Entropy:
      H(P) = - ∑ p_i * ln(p_i)
    subject to physical constraints:
      1. ∑ p_i = 1
      2. Expected single-ball mean = target_mean_sum / 7
    """
    target_ball_mean = target_mean_sum / 7.0

    def objective(p):
        p = np.clip(p, 1e-9, 1.0)
        return np.sum(p * np.log(p))  # Minimize negative entropy = Maximize entropy
...
def signal_recency_regime(df, pool_size=37):
    """
    Models recent appearance regimes using a recency-weighted frequency filter.
    Balls with recent momentum (appeared frequently in last 15 draws AND in
    the most recent draw) get a regime multiplier boost.
    """
    draws = df["numbers"].tolist()
    n_draws = len(draws)
...
def genetic_optimize_weights(df, n_generations=20, pop_size=15):
    ...
    Signals are computed on training data only (excluding last 10 draws) to prevent
    data leakage; the GA validates against the held-out last 10 draws.
    """
    n_val = min(10, len(df) - 20)  # ensure enough training data
    train_df = df.iloc[:-n_val].copy() if n_val > 0 else df.copy()

    # Compute signals on TRAINING data only (no leakage)
    ...
    # Recompute signals on FULL dataset for final prediction output
    signals_full = [
...
def main():
    print("============================================================")
    print("  STEP 10 — ADVANCED QUANTUM & SIGNAL SCIENCE ENGINE (MEGA7)")
    print("============================================================")
    ...
    top_7_indices = np.argsort(quantum_prob)[::-1][:7]
    top_7_numbers = sorted((top_7_indices + 1).tolist())

    print("\n============================================================")
    print("  PHASE 1: QUANTUM SIGNAL SCIENCE PREDICTION")
    print("============================================================")
    print(f"Quantum Science Top 7 Single Predicted Ticket:  ★  {top_7_numbers}  ★")
    ...
    ax2.axis("off")
    summary_text = (
        "★ QUANTUM SCIENCE TOP-7 PREDICTED TICKET ★\n"
        f"  {top_7_numbers}\n\n"
