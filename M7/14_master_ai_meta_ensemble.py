"""
=============================================================
 STEP 14: MASTER A.I. META-ENSEMBLE ENGINE & INFOGRAPHIC (MEGA7)
=============================================================
 LEARNING GOAL:
   1. Harvest and fuse prediction probability vectors from ALL
      previous steps into a unified multi-model feature tensor.
   2. Train an AI Meta-Learner to discover optimal blending weights.
   3. Generate a high-resolution Grand Infographic Chart PNG
      (step14_master_ai_meta_ensemble.png) containing comparative
      model heatmaps, predicted tickets, wheeled tickets, and
      Pro Mathematical Strategy Tips for MEGA7.
=============================================================
"""
...
# ── HARVEST SIGNALS FROM STEPS 1 - 13 + ENHANCEMENTS + GNN & PROCESS ───────────

def harvest_all_signals(df):
    """
    Imports and collects prediction probability vectors from Steps 1 through 13,
...
# ── AI META-LEARNER (NEURAL META-MODEL WEIGHT ADAPTATION) ───────────────────

def meta_ai_blend(signals_dict, df=None):
    """
    Fits Intelligent Meta-Learner Neural Network:
...
def select_optimal_ticket(top_pool: list[int], prob_vector: np.ndarray, ticket_size: int = DRAW_SIZE, df=None) -> list[int]:
    ...
    if df is not None and len(df) >= 10:
        low_counts = df["numbers"].tail(10).apply(lambda nums: sum(1 for n in nums if n <= 18))
...
def main():
    print("============================================================")
    print("  STEP 14 — MASTER A.I. META-ENSEMBLE ENGINE (MEGA7)")
    print("============================================================")

    df = load_data(CSV_FILE)

    print("Harvesting prediction probability vectors across all 13 previous steps...")
    signals_dict = harvest_all_signals(df)

    print("Training Adaptive Tail-Boosted Meta-Learner V2 for MEGA7...")
    ...
    top_7_numbers = select_optimal_ticket(top_14_numbers, meta_prob, ticket_size=DRAW_SIZE, df=df)

    print("\n============================================================")
    print("  GRAND UNIFIED PREDICTION RESULTS (MEGA7)")
    print("============================================================")
    print(f"★ ULTIMATE RECOMMENDED GRAND MASTER TICKET V2:  {top_7_numbers}  ★")
    print(f"Candidate Pool (Top 14 Meta-AI Balls)       : {top_14_numbers}")

    # Validation Depth Metrics
    chi2_res = chi_squared_fit_test(top_7_numbers, df)
    pair_triple_res = calculate_pair_triple_match_rate(top_7_numbers, df["numbers"].iloc[-1])
    ...
    print(f"      • Pair Match Rate vs Last   : {pair_triple_res['pair_match_rate_pct']}% ({pair_triple_res['pairs_hit']}/21 pairs hit)")
    print(f"      • Triple Match Rate vs Last : {pair_triple_res['triple_match_rate_pct']}% ({pair_triple_res['triples_hit']}/35 triples hit)")
    ...
    fig.suptitle("STEP 14 — MASTER A.I. META-ENSEMBLE & PREDICTION DASHBOARD\nEmirates Draw MEGA7",
    ...
    ax2.set_title("Comparative Heatmap: Probability Distributions Across All Quant Models", fontsize=11)
    ...
    summary_text = (
        ...
        "  2. Constrain total ticket sum between 105 and 165.\n"
        "  3. Maintain 3-Zone Minimum Coverage across numbers.\n"
        "  4. Keep High/Low ratio balanced (3 Low, 4 High or 4 Low, 3 High).\n"
    ...
    chart_path = f"{run_dir}/step14_master_ai_meta_ensemble.png"
    ...
    summary_data = {
        ...
        "master_ai_top_7_ticket": [int(n) for n in top_7_numbers],
