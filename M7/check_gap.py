...
from utils import load_data, POOL, DRAW_SIZE

def main():
    print("====================================================================")
    print("  MEGA7 PREDICTION GAP ANALYSIS: 26 JUL 2026 DRAW")
    print("  Winning Combination: [5, 10, 12, 15, 16, 23, 33]")
    print("====================================================================\n")
    ...
    target_win = [5, 10, 12, 15, 16, 23, 33]
    target_set = set(target_win)

    # 1. ANALYZE PROPERTIES OF THE WINNING DRAW
    print("── 1. WINNING DRAW PROPERTY & GAP ANALYSIS ──")
    last_seen_gap = {}
    total_freq = {}
    recent_10_freq = {}
    ...
    # Sum, Odd/Even, Low/High
    win_sum = sum(target_win)
    n_odd = sum(1 for n in target_win if n % 2 != 0)
    n_even = DRAW_SIZE - n_odd
    n_low = sum(1 for n in target_win if n <= 18)
    n_high = DRAW_SIZE - n_low
    z1 = sum(1 for n in target_win if 1 <= n <= 10)
    z2 = sum(1 for n in target_win if 11 <= n <= 20)
    z3 = sum(1 for n in target_win if 21 <= n <= 30)
    z4 = sum(1 for n in target_win if 31 <= n <= 37)
    ...
    print(f"Low/High (<=18 / >18): {n_low} Low / {n_high} High (Notice: Heavy Low Bias!)")
    print(f"Zone Distribution (1-10, 11-20, 21-30, 31-37): {z1}, {z2}, {z3}, {z4}\n")

    # 2. HARVEST MODELS & CHECK PROBABILITY RANKS
    ...
    # Generate Master AI Blend
    p_master, _, _ = mod12.meta_ai_blend(signals_dict, df)
    signals_dict["12. Master AI Meta-Ensemble"] = p_master

    print(f"\n{'Model Name':<28} | {'Top-7 Matches':<14} | {'Top-14 Matches':<15} | {'Avg Win Rank':<12} | {'Missed in Top 14'}")
    print("─" * 105)
    ...
    for name, p_vec in signals_dict.items():
        ...
        m7 = sorted(list(set(top7).intersection(target_set)))
        m14 = sorted(list(set(top14).intersection(target_set)))
        missed14 = sorted(list(target_set - set(top14)))
        
        ranks = []
        for w_num in target_win:
            r = np.where(sorted_nums == w_num)[0][0] + 1
            ranks.append(r)
        avg_rank = np.mean(ranks)
        
        print(f"{name:<28} | {len(m7)}: {str(m7):<11} | {len(m14)}: {str(m14):<12} | {avg_rank:<12.1f} | {missed14}")
    ...
    for w_num in target_win:
        ...
        print(f"Number #{w_num:>2} -> Ranked #{r:>2} out of 37 (Probability: {prob:.2f}%)")

    # Also check what the top 7 and top 14 predictions actually were for Master AI
    print(f"\nMaster AI Top-7 Ticket: {sorted(list(master_sorted[:7]))}")
