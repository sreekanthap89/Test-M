"""
run_all.py — Run all 8 MEGA7 prediction steps in sequence.
...
What it does:
  ...
  2. Runs steps 01 → 08 in order, all charts saved to that folder
...
def collect_results() -> dict:
    ...
    top7    = mod7.diversity_select(ens, k=DRAW_SIZE, candidate_pool=mod7.DEFAULT_POOL_SIZE)
    df_sums = df["numbers"].apply(sum)
    top7    = mod7.validate_sum_range(top7, ens, df_sums, candidate_pool=mod7.DEFAULT_POOL_SIZE)
    ...
    return {
        ...
        "phase1_top7" : sorted([int(x) for x in (np.argsort(p1)[::-1][:DRAW_SIZE] + 1)]),
        "phase2_top7" : [int(x) for x in top7],
        "phase3_top7" : sorted([int(x) for x in (np.argsort(p3)[::-1][:DRAW_SIZE] + 1)]),
        "sum_pattern" : sum_pat,
...
def print_final_report(results: dict, run_folder: str, images: list[str]) -> None:
    ...
    print(f"""
{sep}
  EMIRATES DRAW MEGA7 — FULL RUN COMPLETE (V2 INSTITUTIONAL)
{sep}
...
def main() -> None:
    ...
    print(f"""
{'='*62}
  EMIRATES DRAW MEGA7 — FULL PREDICTION RUN
{'='*62}
